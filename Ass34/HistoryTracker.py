...
# Maintaining File: 1)Date 2)Number of files copied 3)Zip size

# Display using: python Script.py --history
...
def writeHistory(date,fileCount,zipName):
    historyFile="backup_history.txt"

    size=os.path.getsize(zipName)
    sizeKB = round(size / 1024, 2)

    line=date + " | Files: " + str(fileCount) + " | Zip : " + zipName + " | SizeKB : " + str(sizeKB) + "\n"

    fobj=open(historyFile,"a")
    fobj.write(line)
    fobj.close()
...
def marvellousDataShildStart(source="Data"):
    ...
    zipName=makeZip(backupName)

    writeLog("Files copied : " + str(len(files)))
    writeLog("Zip created : " + zipName)


    startTime = time.ctime()
    writeHistory(startTime,len(files),zipName)

    print(border)
    print("Backup completed successfully.")
    print(border)
    print("File copied :",len(files))
    print("Zip file gets created : ",zipName)
    print(border)
